...
class PalObject:
    ...
    def GetImage(self):
        if self._img == None:
            ...
            self._img = tkinter.PhotoImage(
                file=f'{module_dir}/resources/pals/{n}.png')
        return self._img
...
class PalEntity:
    ...
    def SetType(self, value):
        self._obj['CharacterID']['value'] = ("BOSS_" if (
            self.isBoss or self.isLucky) else "") + value
    ...
    def CalculateIngameStats(self):
        ...
        HP_STAT = math.floor(500 + 5 * LEVEL + HP_SCALE *
                             0.5 * LEVEL * (1 + HP_IV))
        HP_STAT = math.floor(HP_STAT * (1 + HP_BONUS) *
                             (1 + HP_SOUL) * (1 + HP_RANK))
        ...
        AT_STAT = math.floor(AT_STAT * (1 + AT_BONUS) *
                             (1 + AT_SOUL) * (1 + AT_RANK))
        ...
        DF_STAT = math.floor(DF_STAT * (1 + DF_BONUS) *
                             (1 + DF_SOUL) * (1 + DF_RANK))
    ...
    def OLD_UpdateMaxHP(self, changes: dict, hp_scaling=None) -> bool:
        ...
        if hp_scaling is None:
            ...
            print("--------")
            print("Derived Specie HP Scaling (from og MaxHP): %s" %
                  possible_hp_scaling)
...
class PalGuid:
    ...
    def GetPlayerslist(self):
        ...
        # {x['value']['NickName']['value']: str(x['uid']['value']) for x in players}
        return out
            ...
            if (e['key'] == GroupGuid):
                for ee in e['value']['RawData']['value']['individual_character_handle_ids']:
                    if (ee['instance_id'] == PalGuid):
                        return
                tmp = {"guid": "00000000-0000-0000-0000-000000000001",
                       "instance_id": PalGuid}
                e['value']['RawData']['value']['individual_character_handle_ids'].append(
                    tmp)
...
class PalPlayerEntity:
    ...
    def SetLifmunkEffigyCount(self, v: int):
        if 'RelicPossessNum' in self._record:
            ...
            self._record['RelicPossessNum'] = {
                'id': None, 'value': v, 'type': 'IntProperty'}
...
with open(os.path.join(module_dir, "resources", "data", "elements.json"), "r", encoding="utf8") as elementfile:
    PalElements = {}
...
def LoadPals(lang="en-GB"):
    ...
    if lang is not None and not os.path.exists(os.path.join(module_dir, "resources", "data", lang, "pals.json")):
        lang = "en-GB"

    with open(os.path.join(module_dir, "resources", "data", "pals.json"), "r", encoding="utf8") as datafile:
        with open(os.path.join(module_dir, "resources", "data", lang, "pals.json"), "r", encoding="utf8") as palfile:
            PalSpecies = {}
            PalLearnSet = {}
            ...
            for i in d["values"]:
                ...
                if t:
                    PalSpecies[i["CodeName"]]._suits = PalSpecies[i["CodeName"].replace(
                        "GYM_", "")]._suits
                    PalSpecies[i["CodeName"]]._scaling = PalSpecies[i["CodeName"].replace(
                        "GYM_", "")]._scaling
                PalLearnSet[i["CodeName"]] = i["Moveset"] if not t else PalLearnSet[i["CodeName"].replace(
                    "GYM_", "")]
